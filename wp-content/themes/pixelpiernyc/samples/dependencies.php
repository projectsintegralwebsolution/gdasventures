...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'Elementor', 'pixelpiernyc' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html( sprintf( __( 'Vamtam Elementor Integration (%s)', 'pixelpiernyc' ), VAMTAM_THEME_SLUG ) ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'pixelpiernyc' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'pixelpiernyc' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'pixelpiernyc' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'pixelpiernyc' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'pixelpiernyc' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'WP Super Cache', 'pixelpiernyc' ),
			'slug'     => 'wp-super-cache',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'pixelpiernyc' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'pixelpiernyc' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'pixelpiernyc' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'pixelpiernyc' ),
		'description' => esc_html__( 'Description', 'pixelpiernyc' ),
		'status'      => esc_html__( 'Status', 'pixelpiernyc' ),
		'version'     => esc_html__( 'Version', 'pixelpiernyc' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'pixelpiernyc' );
	}
	if ( strpos( $plugin['slug'], 'pixelpiernyc' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'pixelpiernyc' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'pixelpiernyc' );
