...
class VamtamFramework {
	...
	public static function theme_supports() {
		...
		function vamtam_elementor_widgets_support( bool $supports, array $args, string $feature ) {
			...
			$theme_features = [
				...
				'nav-menu--submenu-hr-spacing',
				'nav-menu--toggle-sticky-hover-state-on-touch-fix',
				'heading--text-reveal-anim',
				'testimonial-carousel--nav-spacing',
				'image-carousel--nav-spacing',
			];
		...
		if ( function_exists( 'register_nav_menus' ) ) {
			register_nav_menus(
				array(
					'primary-menu'     => esc_html__( 'Menu Header', 'pixelpiernyc' ),
				)
			);
		}
	}
	...
	private function load_languages() {
		load_theme_textdomain( 'pixelpiernyc', VAMTAM_THEME_DIR . 'languages' );
	}
	...
	public static function license_valid() {
		...
		esc_html_e( 'Valid Purchase Key.', 'pixelpiernyc' );
		echo '<p>';
		esc_html_e( 'Congratulations! You have succesfully registered your product.', 'pixelpiernyc' );
		echo '</p>';
		echo '</span>';
	}
	...
	public static function license_invalid() {
		echo '<span id="fail">';
		esc_html_e( 'Incorrect Purchase Key.', 'pixelpiernyc' );
		echo '<p>';
		esc_html_e( 'Please check your purchase code and re-enter.', 'pixelpiernyc' );
		echo '</p>';
		echo '</span>';
	}
	...
	public static function license_failed() {
		...
		esc_html_e( 'Cannot validate Purchase Key. Please try again later. If the problem persists your server might not have the curl PHP extension enabled.', 'pixelpiernyc' );
		echo '</span>';
	}
				...
				<div class="vamtam-license-notice notice">
					...
					<div class="vamtam-notice-content">
						...
						<?php if ( $is_token ) : ?>
							...
							<?php
								echo wp_kses( sprintf( __( ' <a href="%s" target="_blank"> this link</a>', 'pixelpiernyc' ), esc_url( 'https://api.extensions.envato.com/extensions/begin_activation'
	...
	public static function token_unregistered() {
		echo '<span id="success">';
		esc_html_e('Unregistered Token.', 'pixelpiernyc');
		echo '</span>';
	}

	public static function token_unregister_fail() {
		echo '<span id="fail">';
		esc_html_e('Failed to unregister token. Please try again later. If this issue persists please contact support.', 'pixelpiernyc');
		echo '</span>';
	}

	public static function license_unregister() {
		echo '<span id="success">';
		esc_html_e( 'Unregistered Key.', 'pixelpiernyc' );
		echo '</span>';
	}
