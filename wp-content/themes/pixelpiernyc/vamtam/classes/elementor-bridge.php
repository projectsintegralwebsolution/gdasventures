...
class VamtamElementorBridge {
	...
	public static function add_sticky_header_bg_color_to_elementor_global_settings() {
		...
		$kit->add_repeater_row( 'system_colors', [
			...
			'title' => __( 'Sticky Header Bg Color', 'pixelpiernyc' ),
	...
	*/
		...
		if ( $use_el_global_defaults ) {
			...
			for ( $i = 1; $i <= 8; $i++ ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( 'Accent %s', 'pixelpiernyc' ), $i ),
	...
	protected static function add_theme_fonts_to_elementor_global_fonts() {
		...
		if ( $use_el_global_defaults ) {
			...
			// Adding from vamtam_get_option().
			$our_fonts = [
				'primary-font' => __( 'Primary Font', 'pixelpiernyc' ),
				'h1'           => __( 'H1', 'pixelpiernyc' ),
				'h2'           => __( 'H2', 'pixelpiernyc' ),
				'h3'           => __( 'H3', 'pixelpiernyc' ),
				'h4'           => __( 'H4', 'pixelpiernyc' ),
				'h5'           => __( 'H5', 'pixelpiernyc' ),
				'h6'           => __( 'H6', 'pixelpiernyc' ),
			];
			...
			foreach ( $our_fonts as $font_prefix => $font_title ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( '%s', 'pixelpiernyc' ), $font_title ),
	...
	public static function register_locations( $elementor_theme_manager ) {
		...
		$elementor_theme_manager->register_location(
			'page-title-location',
			[
				'label' => esc_html__( 'Page Title', 'pixelpiernyc' ),
				'multiple' => true,
	...
	public static function get_widget_mods_list() {
		...
		*/
		...
		$widget_mods_list = [
			...
			'button'                                     => [ 'label' => __( 'Button', 'pixelpiernyc' ) ],
			'heading'                                    => [ 'label' => __( 'Heading', 'pixelpiernyc' ) ],
			'form'                                       => [ 'label' => __( 'Form', 'pixelpiernyc' ) ],
			'toggle'                                     => [ 'label' => __( 'Toggle', 'pixelpiernyc' ) ],
			'nav-menu'                                   => [ 'label' => __( 'Nav Menu', 'pixelpiernyc' ) ],
			'testimonial-carousel'                       => [ 'label' => __( 'Testimonial Carousel', 'pixelpiernyc' ) ],
			'image-carousel'                       		 => [ 'label' => __( 'Image Carousel', 'pixelpiernyc' ) ],
			'loop-carousel'                              => [ 'label' => __( 'Loop Carousel', 'pixelpiernyc' ) ],
			'search-form'                                => [ 'label' => __( 'Search Form', 'pixelpiernyc' ) ],
			'archive-posts'                              => [ 'label' => __( 'Archive Posts', 'pixelpiernyc' ) ],
			'posts'                                      => [ 'label' => __( 'Posts', 'pixelpiernyc' ) ],
			'post-comments'                              => [ 'label' => __( 'Post Comments', 'pixelpiernyc' ) ],
			'popup' /* Not a widget but same logic */    => [ 'label' => __( 'Popup', 'pixelpiernyc' ) ],
		];
	...
	*/
	...
	public static function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', esc_html__( 'Could not unzip or empty archive.', 'pixelpiernyc' ) );
		}
