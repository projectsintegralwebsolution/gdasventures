...
;
			esc_html_e( 'Hey, just a polite reminder that if you update WordPress you will also need to update your theme and plugins.', 'pixelpiernyc' );
			echo '</strong>';
			echo '</p></div>';
		}
...
;
			esc_html_e( 'VamTam theme resources: ', 'pixelpiernyc' );
			echo '</strong>';
			echo '<a href="https://vamtam.com/child-themes" target="_blank">';
			esc_html_e( 'Sample child themes', 'pixelpiernyc' );
			echo '</a>; ';
			echo '<a href="https://vamtam.com/changelog" target="_blank">';
			esc_html_e( 'Changelog', 'pixelpiernyc' );
			echo '</a>';
			echo '</p></div>';
		}
	}
	...
	public static function icons_selector() {
		...
		<div class="vamtam-config-icons-selector hidden">
			<input type="search" placeholder="<?php esc_attr_e( 'Filter icons', 'pixelpiernyc' ) ?>" class="icons-filter"/>
