...
class VamtamPurchaseHelper extends VamtamAjax {
	...
	private static function server_tests() {
		...
		$tests = array(
			array(
				'name'  => esc_html__( 'PHP Version', 'pixelpiernyc' ),
				'test'  => version_compare( phpversion(), '5.5', '<' ),
				'value' => phpversion(),
				'desc'  => esc_html__( 'While this theme works with all PHP versions supported by WordPress Core, PHP versions 5.5 and older are no longer maintained by their developers. Consider switching your server to PHP 5.6 or newer.', 'pixelpiernyc' ),
			),
			array(
				'name'  => esc_html__( 'PHP Time Limit', 'pixelpiernyc' ),
				'test'  => $timeout > 0 && $timeout < 30,
				'value' => $timeout,
				'desc'  => esc_html__( 'The PHP time limit should be at least 30 seconds. Note that in some configurations your server (Apache/nginx) may have a separate time limit. Please consult with your hosting provider if you get a time out while importing the demo content.', 'pixelpiernyc' ),
			),
			array(
				'name'  => esc_html__( 'PHP Memory Limit', 'pixelpiernyc' ),
				'test'  => (int) $memory > 0 && $memoryB < 96 * 1024 * 1024,
				'value' => $memory,
				'desc'  => esc_html__( 'You need a minimum of 96MB memory to use the theme and the bundled plugins. For non-US English websites you need a minimum of 128MB in order to accomodate the translation features which are otherwise disabled.', 'pixelpiernyc' ),
			),
			array(
				'name'  => esc_html__( 'PHP ZipArchive Extension', 'pixelpiernyc' ),
				'test'  => ! class_exists( 'ZipArchive' ),
				'value' => '',
				'desc'  => esc_html__( 'ZipArchive is a requirement for importing the demo sliders.', 'pixelpiernyc' ),
			),
		);
	...
	public static function tgmpa_plugins_complete_actions( $update_actions, $plugin_info ) {
		if ( isset( $update_actions['dashboard'] ) ) {
			$update_actions['dashboard'] = sprintf(
				esc_html__( 'All plugins installed and activated successfully. %1$s', 'pixelpiernyc' ),
				'<a href="' . esc_url( admin_url( 'admin.php?page=vamtam_theme_setup_import_content' ) ) . '" class="button button-primary">' . esc_html__( 'Continue with theme setup.', 'pixelpiernyc' ) . '</a>'
			);
	...
	public static function admin_menu() {
		add_menu_page( esc_html__( 'VamTam', 'pixelpiernyc' ), esc_html__( 'VamTam', 'pixelpiernyc' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ), '', 2 );
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Dashboard', 'pixelpiernyc' ), esc_html__( 'Dashboard', 'pixelpiernyc' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ) );
	...
	public static function admin_menu_1() {
		...
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Import Demo Content', 'pixelpiernyc' ), esc_html__( 'Import Demo Content', 'pixelpiernyc' ), 'edit_theme_options', 'vamtam_theme_setup_import_content', array( __CLASS__, 'vamtam_theme_setup_import_content' ) );
	}
	...
	public static function registration_warning() {
		...
		<div class="vamtam-notice-wrap">
			<div class="vamtam-notice">
				<p>
					<?php echo esc_html__( 'Please activate your license to get theme updates, premium support, and access to demo content.', 'pixelpiernyc' ); ?>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=vamtam_theme_setup' ) ); ?>">
						<?php echo esc_html__( 'Register Now', 'pixelpiernyc' ); ?>
					</a>
				</p>
			</div>
		</div>
	...
	public static function admin_init() {
		...
		add_settings_field(
			...
			esc_html__( 'Enter your purchase code from ThemeForest to receive theme updates and support.', 'pixelpiernyc' ),
		...
		<div id="vamtam-ts-homepage" class="vamtam-ts">
			...
			<div id="vamtam-ts-main">
					...
					<div id="vamtam-ts-register-product">
						<?php
							if ( defined( 'ENVATO_HOSTED_SITE' ) ) :
								esc_html_e( 'All done.', 'pixelpiernyc' );
							else :
						?>
							<form id="vamtam-register-form" method="post" action="options.php" autocomplete="off">
								<?php if ( $valid_key ) : ?>
									<div id="vamtam-verified-code">
										<p>
											<?php  esc_html_e( 'Thanks for the verification!', 'pixelpiernyc' ) ?>
											<br />
											<?php echo esc_html( sprintf( __( 'You can now enjoy %s and build great websites.', 'pixelpiernyc' ) , $theme_name ) ); ?>
										</p>
									</div>
								<?php else : ?>
				...
				<div id="vamtam-check-license-disclaimer">
					<h5><?php esc_html_e( 'Licensing Terms', 'pixelpiernyc' ); ?></h5>
					<p>
						<?php esc_html_e( 'You need to register a separate license for each domain on which you will use the theme. A single license is limited to a single domain/application. For more information, please refer to these articles - ', 'pixelpiernyc' ); ?>
						<a href="http://themeforest.net/licenses" target="_blank">
							<?php esc_html_e( 'Licensing Terms Envato Market', 'pixelpiernyc' ); ?>
						</a>,
						<a href="https://elements.envato.com/license-terms" target="_blank">
							<?php esc_html_e( 'Licensing Terms Envato Elements', 'pixelpiernyc' ); ?>
						</a>
						.
					</p>
				</div>
						...
						<div id="vamtam-server-tests">
							<h3>
								<?php if ( $status['fail'] > 0 ) : ?>
									<?php esc_html_e( 'System Status', 'pixelpiernyc' ) ?>
									<?php $fail = $status['fail']; ?>
									<small><?php printf( esc_html( _n( '(%d potential issue)', '(%d potential issues)', $fail, 'pixelpiernyc' ) ), $fail ) ?></small>
								<?php endif ?>
							</h3>
						</div>
					<?php endif ?>
	...
	public static function dashboard_navigation() {
		...
		<nav id="vamtam-ts-nav-menu">
			<div id="vamtam-theme-title">
				<span id="vamtam-ts-greeter"><?php esc_html_e( 'WELCOME TO', 'pixelpiernyc' ); ?></span>
				<span id="vamtam-ts-greeter-title"><?php echo esc_html( $theme_name ); ?></span>
				<span id="vamtam-ts-greeter-ver"><?php echo sprintf( esc_html__( 'VER. %s', 'pixelpiernyc' ), $theme_version ); ?></span>
			</div>
			<ul>
				<li class="<?php echo esc_attr( $cur_route === 'toplevel_page_' . $routes[0] ? 'is-active' : '' ); ?>" >
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[0] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Register' , 'pixelpiernyc' ); ?></span>
				...
				<?php if ( isset( $tgmpa_instance ) && isset( $tgmpa_instance->page_hook ) ) : ?>
					<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[1] ? 'is-active' : '' ); ?>" >
						<a <?php echo esc_attr( ! $valid_key ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[1] ) ); ?>">
							...
							<span><?php echo esc_html__( 'Install Plugins' , 'pixelpiernyc' ); ?></span>
				...
				<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[2] ? 'is-active' : '' ); ?>" >
					<a <?php echo esc_attr( ! $valid_key || $plugin_status !== 'success' ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[2] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Import Demo' , 'pixelpiernyc' ); ?></span>
			...
			<a id="vamtam-hs-btn" class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[3] ? 'is-active' : ''); ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[3] ) ); ?>">
				...
				<span><?php echo esc_html__( 'Help & Support' , 'pixelpiernyc' ); ?></span>
			</a>
		</nav>
		<?php
	}
	...
	public static function import_buttons() {
		...
		$messages = array(
			'success-msg' => esc_html__( 'Imported.', 'pixelpiernyc' ),
			'error-msg  ' => esc_html__( 'Failed to import. Please <a href="{fullimport}" target="_blank">click here</a> in order to see the full error message.', 'pixelpiernyc' ),
		);

		$import_tests = array(
			array(
				'test'   => defined( 'ELEMENTOR_PRO__FILE__' ),
				'title'  => esc_html__( 'Posts, Pages and Site Layout', 'pixelpiernyc' ),
				'failed' => wp_kses( __( "This theme requires Elementor Pro. If you don't have Elementor Pro, please <a href='https://be.elementor.com/visit/?bta=13981&nci=5383' target='_blank'>download it here</a>. Install and activate it, and then proceed with importing the demo content. If you have any issues with the importer please <a href='https://elementor.support.vamtam.com/support/solutions/articles/245218-vamtam-elementor-themes-how-to-install-the-theme-via-the-admin-panel-' target='_blank'>read this article</a> or reach out to us using <a href='https://themeforest.net/user/vamtam' target='_blank'>the form on this page</a>.", 'pixelpiernyc' ), 'vamtam-a-span' ),
			),
		);
		...
		$img_progress = $total_attachements > 0 && class_exists( 'Vamtam_Importers_E' ) && is_callable( [ 'Vamtam_Importers_E', 'get_attachment_progress' ] ) ?
			...
			esc_html__( 'checking...', 'pixelpiernyc' );
		...
		$buttons = array(
			array(
				'label'          => esc_html__( 'Dummy Content Import', 'pixelpiernyc' ),
				'id'             => 'content-import-button',
				'description'    => esc_html__( 'You are advised to use this importer only on new WordPress sites.', 'pixelpiernyc' ),
				'button_title'   => esc_html__( 'Import', 'pixelpiernyc' ),
				...
				'data'           => array_merge( $messages, [
					...
					'success-msg'      => sprintf( esc_html__( 'Main content imported. Image import progress: <span class="vamtam-image-import-progress">%s</span>.', 'pixelpiernyc' ), $img_progress ),
					'fail-msg'         => esc_html__( 'Failed to import. We recommend that you contact your hosting provider for advice, as solving this issue is often specific to each server.', 'pixelpiernyc' ),
					'timeout-msg'      => esc_html__( 'Failed to import. This is most likely caused by a timeout. Please contact your hosting provider for advice as to how you can increase the time limit on your server.', 'pixelpiernyc' ),
				] ),
				'additional_msg' => $import_disabled_msg . wp_kses( sprintf( __( '<p class="vamtam-description">Please make sure to <a href="%s" target="_blank">backup</a> any existing content that you need as it will be removed by the import procedure (affects Posts, Pages and Menus).</p><p class="vamtam-description">We recommend that you use the <a href="%s" target="_blank">Post Name permalink structure</a></p><p class="vamtam-description">Images will be downloaded in the background after the main import is complete. Depending on your server, this may take several minutes.<br> In the meantime you may notice that some images are not visible.', 'pixelpiernyc' ), esc_url( admin_url( 'plugin-install.php?tab=plugin-information&plugin=updraftplus&TB_iframe=true&width=772&height=921' ) ), esc_url( admin_url( 'options-permalink.php' ) ) ), 'vamtam-admin' ),
				'disabled_msg_plain' => '',
			),
		);
	...
	public static function purchase_key( $args ) {
		...
		$placeholder = __( 'XXXXXX-XXX-XXXX-XXXX-XXXXXXXX', 'pixelpiernyc' );
		$plugin_status = VamtamPluginManager::get_required_plugins_status();
		$content_imported = ! ! get_option( 'vamtam_last_import_map', false );
		...
		if ( $valid_key ) {
			...
			esc_html_e( 'Invalid', 'pixelpiernyc' );
			echo '</span>';
			echo '</span>';
		}
		...
		if ( ! defined( 'ENVATO_HOSTED_SITE' ) ) {
			...
			if ( ! $valid_key ) {
				...
				echo '<label for="vamtam-envato-elements-cb">' . __( 'I downloaded the theme from Envato Elements.', 'pixelpiernyc' ) . '</label>';
				echo '</div>';

				echo '<p id="vamtam-code-help">' . wp_kses( sprintf( __( ' <a href="%s" target="_blank">Where can I find my Item Purchase Code?</a>', 'pixelpiernyc' ), 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ), 'vamtam-a-span' ) . '</p>';

				echo '<p id="vamtam-code-help-elements" class="hidden">';
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Follow this link to generate a new Envato Elements Token.</a>', 'pixelpiernyc' ), esc_url( 'https://api.extensions.envato.com/extensions/begin_activation'
				...
				echo '>' . esc_html__( 'Register', 'pixelpiernyc' );
				echo '</button>';
				...
				echo esc_html__( 'Continue to required plguins', 'pixelpiernyc' );
				echo '</a>';
				...
				echo esc_html__( 'Continue to demo import', 'pixelpiernyc' );
				echo '</a>';
			}
